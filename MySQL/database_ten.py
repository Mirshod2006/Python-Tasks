...
mycursor.executemany("INSERT INTO flights (id,plane_type,time,take_off,take_on) VALUES (%s,%s,%s,%s,%s)",[
...
mycursor.execute("DELETE FROM flights WHERE SUBSTRING(time,12,2)='14' AND SUBSTRING(time,12,2)='17' AND LOCATE('Tashkent',take_off)")
mydb.commit()

mycursor.execute("SELECT * FROM flights WHERE SUBSTRING(time,6,2)='09' OR SUBSTRING(time,6,2)='10' OR SUBSTRING(time,6,2)='11'")
